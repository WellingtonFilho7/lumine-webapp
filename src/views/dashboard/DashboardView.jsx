  ...
  return (
    <div className="space-y-4">
      <div className="space-y-3">
        <StatCard value={stats.present} label="Presentes" color="green" icon={CheckCircle} size="lg" />
        <div className="grid grid-cols-3 gap-2">
          ...
          <StatCard value={stats.meals ?? 0} label="Refeições" color="amber" icon={Calendar} size="sm" />
        </div>
      </div>
      ...
      <div className="rounded-lg bg-white p-4 shadow-md">
        ...
        {pendingToday.length > 0 ? (
          <div className="space-y-2">
            ...
            >
              {pendingToday.length > 5 ? `Ver todos (${pendingToday.length})` : 'Ir para registros'}
            </button>
          </div>
        ) : (
